...
asmlinkage long interceptor(struct pt_regs reg) {
    ...
    switch (monitored) {
        ...
        case 2:
            /* This condition is for black-list checking. */
            if (check_pid_monitored(reg.ax, current->pid)==1) {
                break;
            }
            log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
 ...
 * 
 * Handle all 4 commands correctly.
 *
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
    ...
    /* Handle invalid syscall numbers. */
    if (syscall<0 || syscall>NR_syscalls || syscall==MY_CUSTOM_SYSCALL) {
        return -EINVAL;
    }
    
    /* Dealing with first two commands. */
    if (cmd==REQUEST_SYSCALL_INTERCEPT || cmd==REQUEST_SYSCALL_RELEASE) {
        ...
        /* Handle intercept request if root. */
        if (cmd==REQUEST_SYSCALL_INTERCEPT) {
            ...
            /* Make the system call table rw, save the original call, make the table ro. */
        ...
        if (cmd==REQUEST_SYSCALL_RELEASE){
            ...
            /* Verify the system call was intercepted. Restore original system call if true. */
            spin_lock(&calltable_lock);
            spin_lock(&pidlist_lock);
            ...
            /* Update the mytable table entry. */
            table[syscall].intercepted = 0;
            
            /* Delete the list of the monitored pids. */
            destroy_list(syscall);
            INIT_LIST_HEAD(&(table[syscall].my_list));
            spin_unlock(&pidlist_lock);
            
            /* Restore the original system call into the system call table. */
            set_addr_rw((unsigned long)sys_call_table);
    ...
    /* Dealing with last two commands now. */
    ...
    if (cmd==REQUEST_START_MONITORING || cmd==REQUEST_STOP_MONITORING) {
        ...
        /* Cannot monitor a pid if the system call is not intercepted, and pid
         cannot be negative. */
       
        spin_lock(&calltable_lock);
        spin_lock(&pidlist_lock);
        if ((table[syscall].intercepted==0) || pid < 0) {
            spin_unlock(&pidlist_lock);
            spin_unlock(&calltable_lock);
            return -EINVAL;
        }
        
        /* Handle not root condition. */
        
        if (current_uid()!=0) {
            
            /* We're not root and either pid is 0, or calling process does not own
             the pid. */
        ...
        if (cmd==REQUEST_START_MONITORING) {
            ...
            /* If pid is 0 and there exists pids being monitored for the system call,
             delete the entries in the list to make it a black list. */
            
            if (pid==0) {
                
                /* If pid==0 and the blacklist is empty, ALL pid's are already monitored. */
                ...
                /* Some pid's are blacklisted for the system call, so clear the monitored list,
            ...
            /* HANDLE ADDING A PID TO BLACKLIST, ADD PID TO MONITOR(IF MONITORED!=2) */
        ...
        if (cmd==REQUEST_STOP_MONITORING) {
            ...
            /* If we want to stop monitoring all pids, update the monitored flag,
             */
...
static int init_function(void) {
    ...
    /* Save the ORIGINAL custom system call, and the ORIGINAL exit call. */
